...
get_mutation_rate <- function(type, anno) {
	inputdir <- "/oak/stanford/groups/ccurtis2/users/khoulaha/BRCA_drivers/mutations"
	if (type == 'snv') {
		filename <- file.path(inputdir, 'Metabric_snv_count.txt')
	} else if (type == 'cnabp') {
		filename <- file.path(inputdir, 'Metabric_cna_bps_count.txt')
	} else if (type == 'cnaseg') {
		filename <- file.path(inputdir, 'Metabric_cna_seg_count.txt')
	} else if (type == 'cnagene') {
		filename <- file.path(inputdir, 'Metabric_cna_count.txt')
	} else if (type == 'lossbp') {
		filename <- file.path(inputdir, 'Metabric_losses_bps_count.txt')
	} else if (type == 'lossseg') {
		filename <- file.path(inputdir, 'Metabric_losses_seg_count.txt')
	} else if (type == 'lossgene') {
		filename <- file.path(inputdir, 'Metabric_losses_count.txt')
	} else if (type == 'gainbp') {
		filename <- file.path(inputdir, 'Metabric_gains_bps_count.txt')
	} else if (type == 'gainseg') {
		filename <- file.path(inputdir, 'Metabric_gains_seg_count.txt')
	} else if (type == 'gaingene') {
		filename <- file.path(inputdir, 'Metabric_gains_count.txt')
	} else {
	 	stop("Invalid mutation type specified. Please specifiy either snv, cnabp, cnaseg, lossbp, lossseg, gainbp, gainseg ...")
	}
	# read in mutations 
	mut <- read.delim(
		filename,
	...
	# reformat anno 
	anno <- anno[,c('PATIENT_ID','AGE_AT_DIAGNOSIS')]
	colnames(anno) <- c('sample','age')
	...
	mut_rate$rate <- mut_rate$count/as.numeric(mut_rate$age)
	rownames(mut_rate) <- mut_rate$sample
	return(mut_rate)
}
...
calculate_mutation_rate_ratio <- function(int, mut_rate, ddr, wt, anno, cancer) {
	...
	# sample wt based on PAM50 and stage that matches ddr
...
calculate_median_est_incidence <- function() {
	...
	par <- expand.grid(
		gene = c('BRCA1','BRCA2'), 
		mutation = c('cnagene','lossgene','gaingene')
		#mutation = c('snv','cnabp', 'lossbp','gainbp', 'cnaseg', 'lossseg','gainseg')
		)
	...
	incidence <- apply(
		...
		1,
		function(x) {
			filename <- paste(date, x['gene'], x['mutation'], 'incidence_estimates.tsv', sep = '_') 
			print(filename)
			...
			ob_median <- observed[observed$Cancer == 'BRCA' & observed$Gene == x['gene'],'Median']
			ob_L95 	<- observed[observed$Cancer == 'BRCA' & observed$Gene == x['gene'],'L95']
			ob_U95 	<- observed[observed$Cancer == 'BRCA' & observed$Gene == x['gene'],'U95']
			ob_n 	<- observed[observed$Cancer == 'BRCA' & observed$Gene == x['gene'],'Num']
			# calculate standard deviation 
			ob_sd <- sqrt(ob_n)*(ob_U95 - ob_L95)/3.92
			ob_dist <- rnorm(10000, mean = ob_median, sd = ob_sd)
			plots <- create_incidence_segplot(tmp, ob_median = ob_median, ob_L95 = ob_L95, ob_U95 = ob_U95, 
				filename = paste(date, x['gene'], x['mutation'], 'segplot.tiff', sep = '_'), 
				main = x['gene'], driver_max = 15, ylimit = 60, yat = seq(0,60,20))
			# calculate median 
...
create_incidence_segplot <- function(tmp, ob_median, ob_L95, ob_U95, filename, main, 
	...
	plot_data <- data.frame(num = 1:length(mean_inc), mean = mean_inc, 
	...
	# set xlab 
	mut <- strsplit(filename, '_')[[1]][3]
	labels <- c(
		'Number SNV drivers',
		rep(c('Number CNA drivers','Number CNA deletion drivers','Number CNA gain drivers'), 3)
		)
	names(labels) <- c('snv','cnabp','lossbp','gainbp','cnaseg','lossseg','gainseg','cnagene','lossgene','gaingene')
	# create segplot
	create.scatterplot(
		mean ~ num,
		data = plot_data, 
...
standardize_clinical_characteristics_breast <- function(anno, wt, ddr) {
	# add pam50 and stage groups 
	anno$group <- factor(paste(anno$pam50, anno$stage, sep = '|'))
	# split into ddr and wt
	anno_wt <- anno[anno$PATIENT_ID %in% wt,]
	anno_ddr <- anno[anno$PATIENT_ID %in% ddr,]
	# find weights 
	group_weights <- as.data.frame(table(anno_ddr$group))
	# get weights of each group 
	colnames(group_weights) <- c('group','prop')
	group_weights$prop <- group_weights$prop/nrow(anno_ddr)
	# add weights 
	anno_wt <- merge(anno_wt, group_weights, by = 'group')
	# randomly sample wt 
	wt_sample <- sample_n(anno_wt, size = length(ddr), weight = anno_wt$prop)
	return(wt_sample$PATIENT_ID)
	}
...
calculate_mut_rate_cna_snv_meth_clones <- function(anno) {
	...
	mut_rate 				<- rbind(mut_rate_snv, mut_rate_cna, mut_rate_del, 
