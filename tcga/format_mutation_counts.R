...
### DATA PROCESSING ###############################################################################
# read in cna data 
segments <- read.delim('TCGA_mastercalls.abs_segtabs.fixed.txt.gz')
...
write.table(
	del_dedup,
	file = 'TCGA_segments_deletions_only.tsv',
	sep = '\t',
	row.names = FALSE,
	quote = FALSE
	)
...
### FORMAT INDELS #################################################################################
# read in mc3 
mc3 <- read.delim('mc3.v0.2.8.PUBLIC.maf.gz', as.is = TRUE)
...
write.table(
 	indel_df, 
 	file = 'TCGA_indels_deletions_bp_only.tsv', 
 	sep = '\t', 
 	row.names = FALSE, 
 	quote = FALSE
 	)
...
write.table(
 	ins_df, 
 	file = 'TCGA_indels_insertions_bp_only.tsv', 
 	sep = '\t', 
 	row.names = FALSE, 
 	quote = FALSE
 	)
